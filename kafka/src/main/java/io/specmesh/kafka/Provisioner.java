...
public final class Provisioner {
    ...
    public static void provisionTopicsAndSchemas(final KafkaApiSpec apiSpec,
        ...
        final Collection<String> existingTopics = adminClient.listTopics().listings().get(WAIT, TimeUnit.SECONDS).stream().map(topicListing -> topicListing.name()).collect(Collectors.toList());
        final List<NewTopic> newTopicsToCreate = domainTopics.stream().filter(newTopic -> !existingTopics.contains(newTopic.name())).collect(Collectors.toList());
    ...
    private static void provisionSchemas(final KafkaApiSpec apiSpec,
        ...
        domainTopics.forEach((topic -> {
            ...
            try {
                final Path path = Paths.get(baseResourcePath + "/" + schemaRef);
                System.out.println("GOT:" + path.toAbsolutePath());

                schemaContent = readString(path, UTF_8);
            } catch (IOException e) {
